...
def create_user_dict():
    ...
    return_dict['delay_total_query'] = 0.0
    return_dict['delay_doc_total'] = 0.0
    return_dict['nothing'] = 0.0
...
def generate_output(summaries, separator=' ', include_header=True):
    ...
    if len(summaries) == 0:
        ...
        for user in summaries:
            ...
            for key in OUTPUT_KEYS:
                user_str += '{0}{1}'.format(summaries[user][key], separator)
...
def generate_user_summaries(input_file):
    ...
    for line in input_file:
        ...
        summaries[user]['page_count'] += int(split_line[5])
        summaries[user]['doc_count'] += int(split_line[6])
        summaries[user]['doc_examined_depth'].append(int(split_line[7]))
        summaries[user]['docs_marked_relevant_count'] += int(split_line[8])
        summaries[user]['doc_relevant_depth'].append(int(split_line[9]))
        summaries[user]['doc_hover_count'] += int(split_line[10])
        summaries[user]['doc_hover_depth'].append(int(split_line[11]))
        summaries[user]['docs_trec_relevant_count'] += int(split_line[12])

        summaries[user]['time_total_query'] += float(split_line[13])
        summaries[user]['time_total_session'] += float(split_line[14])
        summaries[user]['time_total_doc'] += float(split_line[15])
        summaries[user]['time_total_serp'] += float(split_line[16])
        summaries[user]['time_total_serp_lag'] += float(split_line[17])
        summaries[user]['delay_total_query'] += float(split_line[18])
        summaries[user]['delay_doc_total'] += float(split_line[19])
        summaries[user]['nothing'] += float(split_line[20])
    ...
    for user in summaries:
        ...
        summaries[user]['delay_per_query'] = summaries[user]['delay_total_query'] / q
        summaries[user]['delay_doc_per_doc'] = summaries[user]['delay_doc_total'] / d
